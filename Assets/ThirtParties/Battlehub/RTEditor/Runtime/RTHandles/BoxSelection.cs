...
namespace Battlehub.RTHandles
{
    ...
    public class BoxSelection : RTEComponent, IBoxSelection
    {
        ...
        protected override void Awake()
        {
            ...
            m_selectionRenderer = new SelectionPicker(Window, args => Filtering?.Invoke(this, args));
        }
        ...
        {
            if(Editor.Tools.ActiveTool != null || m_windowRectTransform == null)
            {
                return;
            }
        ...
        private void HitTest()
        {
            ...
            Renderer[] renderers = UnityObjectExt.FindObjectsByType<Renderer>();

            if(MethodOverride == BoxSelectionMethod.PixelPerfectDepthTest)
            {
                IEnumerable<Renderer> selectedRenderers = m_selectionRenderer.PixelPerfectDepthTest(renderers, SelectionBounds);
                IEnumerable<Renderer> filteredRenderers = FiterRenderers(selectedRenderers, filteringArgs);
                selection = new HashSet<GameObject>(filteredRenderers.Select(rend => rend.gameObject));
            }
            else
            {
            ...
            SpriteGizmo[] spriteGizmos = UnityObjectExt.FindObjectsByType<SpriteGizmo>();
            for (int i = 0; i < spriteGizmos.Length; ++i)
            {
                ...
                if (select)
                {
                    FilterGameObjects(selection, filteringArgs, spriteGizmo.gameObject);
                }
            }
